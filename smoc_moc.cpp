...
void smoc_scheduler_top::scheduleSR(smoc_graph *c) {
  ...
  do {
    ...
    */
	...
	assert( status == transition_ty::ENABLED /*|| status == transition_ty::DISABLED */);
	//	if( status == transition_ty::DISABLED ){ // do not treat disabled transitions
	...
	assert( status == transition_ty::ENABLED || status == transition_ty::DISABLED );
	if( status == transition_ty::DISABLED ){ // do not execute disabled transitions
	...
	while(nonStrictReleased){
	  ...
	  smoc_root_node                   &n = transition.getActor(); //FIXME(MS): rename n -> node, block..
	  assert(n.isNonStrict());
	  if( transition_ty::ENABLED == transition.getStatus()){
	    ...
	    assert( transition_ty::DISABLED == transition.getStatus());
	    temp      |=  transition;
	  }
