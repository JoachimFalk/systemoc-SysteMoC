...
operator<<(boost::numeric::ublas::vector<T,A>& input_vector, 
					 const T& data_el){
	typename boost::numeric::ublas::vector<T,A>::size_type old_size(input_vector.size());
...
public:
    ...
    : parent_type(size)
  {
	...
	BOOST_UBLAS_INLINE
	template <int N>
  smoc_vector(const smoc_vector_init::smoc_vector_init<size_type,N>& 
							vector_init)
    : parent_type(N)
  {
...
public:
  ...
  BOOST_UBLAS_INLINE
  smoc_vector_cs(const smoc_vector_init::smoc_vector_init<size_type,N>& 
								 vector_init)
    : parent_type(N)
  {
    for(size_type i = 0; i < N; i++){
...
namespace smoc_vector_init{
	extern smoc_vector_init<smoc_vector_cs<int,1>::size_type > vector_init;
};


template <class VECTOR_CLASS, class A, typename ID_TYPE, int N>
boost::numeric::ublas::vector<VECTOR_CLASS, A>& 
operator<<(boost::numeric::ublas::vector<VECTOR_CLASS,A>& input_vector, 
					 const smoc_vector_init::smoc_vector_init<ID_TYPE,N>& vector_init){
	typename boost::numeric::ublas::vector<VECTOR_CLASS,A>::size_type old_size(input_vector.size());

	std::cout << "input_vector = " << input_vector << std::endl;
	std::cout << "vector_init = " << VECTOR_CLASS(vector_init) << std::endl;

	input_vector.resize(old_size+1);
	input_vector[old_size] = VECTOR_CLASS(vector_init);
