...
To express the behavior of a firing step we need to define the prefix order $\sqsubseteq$ on the set of sequences $S$, i.e., given $\mathbf{u}, \mathbf{v} \in S$ then $\mathbf{u} \sqsubseteq \mathbf{v} \equiv \mathbf{u}.\mathbb{N} \subseteq \mathbf{v}.\mathbb{N} \wedge \forall{n \in \mathbf{u}.\mathbb{N}}: \mathbf{u}(n) = \mathbf{v}(n)$.
...
\begin{eqnarray}
\mathrm{Fire}(\mathbf{s}_\mathrm{in}, q) = \{%
 &        & (\mathbf{s}_\mathrm{prod}, q') \\
 & |      & (q.q_\mathrm{firing}, k, f_\mathrm{action}, q'.q_\mathrm{firing}) \in T \\
 & \wedge & \mathbf{s}_\mathrm{cons} \sqsubseteq \mathbf{s}_\mathrm{in} \\
 & \wedge & k(\mathbf{s}_\mathrm{cons}, q.q_\mathrm{func}) \\
 & \wedge & \forall{\mathbf{s}_\mathrm{prefix} \sqsubset \mathbf{s}_\mathrm{cons}}: \neg k(\mathbf{s}_\mathrm{prefix}, q.q_\mathrm{func}) \\
 & \wedge & (\pi{}_{O}(\mathbf{s}_\mathrm{prod}), q'.q_\mathrm{func}) = f_\mathrm{action}(\pi{}_{I}(\mathbf{s}_\mathrm{cons}), q.q_\mathrm{func}) \\
 & \wedge & \#\pi{}_{I}(\mathbf{s}_\mathrm{prod}) = \#\pi{}_{I}(\mathbf{s}_\mathrm{cons}) \}
\end{eqnarray}

The possible firing steps an actor can take are dependent on the its state $q$ and the tuple $\mathbf{s}_\mathrm{in}$ of still unconsumed input sequences.

The $\mathrm{Fire}$-function defined above r
Each firing step possible from 

transforms \emph{fire function}.

, i.e., $\mathrm{Fire}: \mathbf{S}_\mathrm{in} \times Q \to 2^{\mathbf{S}_\mathrm{out} \times Q}$.
