...
The behavior of an actor $a$ can be thought of as a relation $\mathbb{A}$ which maps a tuple of input sequences $\mathbf{s}_\mathrm{in} \in \mathbf{S}_\mathrm{in} \subset S^{|\mathcal{P}|}$ into a tuple of output sequences $\mathbf{s}_\mathrm{out} \in \mathbf{S}_\mathrm{out} \subset S^{|\mathcal{P}|}$, i.e., $\mathbb{A} \subseteq  \mathbf{S}_\mathrm{in} \times \mathbf{S}_\mathrm{out}$, where $\mathcal{P}$ denotes the set of all ports of the actor $a$, i.e., $\mathcal{P} = I \cup O$.
...
A \emph{projection} $\pi{}_P: S^n \to S^m$ is a function which discards and reorders sequences according to the ordered set of actor ports $P \subseteq \mathcal{P}$ in a $n$-tuple of sequences to form a new $m$-tuple of sequences, where $|P| = m \le n$.
In other words, given $\mathbf{s} = (\mathbf{v}_{p'_1},\ldots,\mathbf{v}_{p'_n}) \in S^n$ and $P = \{p_1,\ldots,p_m\} \subseteq \{p'_1,\ldots,p'_n\}$ then $\pi{}_P(\mathbf{s}) = (\mathbf{v}_{p_1}, \ldots, \mathbf{v}_{p_m})$.
%A projection $\pi{}_P$ can be generalized to subsets $\mathbf{X}$ of $S^n$, i.e., $\pi{}_P(\mathbf{X}) = \{\pi{}_P(\mathbf{s}) \mid \mathbf{s} \in \mathbf{X}\}$.
%\end{definition}
...
Furthermore a tuple can be viewed as a function, e.g., $\mathbf{s}: \mathbb{N}_3 \to S$, over the counting set $\mathbb{N}_n = \{1,2,\ldots,n\}$ from one to the tuple size $n$.
...
The size of a tuple will be denoted via the $\#$-operator, e.g., $\#\mathbf{s} = |\mathbf{s}.\mathbb{N}| = 3$.

With the mathematical notation defined above it is possible to describe further constraints on the tuple of input and output sequences.
The tuple of input sequences $\mathbf{s}_\mathrm{in}$ is constraint to be devided into value sequences $\pi{}_{I}(\mathbf{s}_\mathrm{in}) \in (V^{**})^{|I|}$ and free space symbol sequences $\pi{}_{O}(\mathbf{s}_\mathrm{in}) \in (\{\circ\}^{**})^{|O|}$.
...
Additionally we define a prefix order $\sqsubseteq$ on the set of sequences $S$, i.e., given $\mathbf{v}_1, \mathbf{v}_2 \in S$ then $\mathbf{v}_1 \sqsubseteq \mathbf{v}_2 \equiv \mathbf{v}_1.\mathbb{N} \subseteq \mathbf{v}_2.\mathbb{N} \wedge \forall{n \in \mathbf{v}_1.\mathbb{N}}: \mathbf{v}_1(n) =  \mathbf{v}_2(n)$.
This prefix order has a trivial pointwise extenstion to tuples of sequences, i.e., given $\mathbf{s}_1, \mathbf{s}_2 \in S^n$ then $\mathbf{s}_1 \sqsubseteq \mathbf{s}_2 \equiv \forall{m \in \mathbb{N}_n}: \mathbf{s}_1(m) \sqsubseteq \mathbf{s}_2(m)$.

Each firing step of an actor consumes a tuple of finite sequences $\mathbf{s}_\mathrm{cons} \in \mathbf{S}_\mathrm{in}$, which must be a prefix of the tuple of input sequences, i.e., $\mathbf{s}_\mathrm{cons} \sqsubseteq \mathbf{s}_\mathrm{in}$.
Consequently a tuple of output sequences $\mathbf{s}_\mathrm{prod} \in \mathbf{S}_\mathrm{out}$ is produced.
Furthermore each firing step transforms the \emph{actor state} $s \in S = \mathcal{F}.S_\mathrm{func} \times \mathcal{R}.S_\mathrm{firing}$, i.e., $\mathrm{Fire}: \mathbf{S}_\mathrm{in} \times S \to 2^{\mathbf{S}_\mathrm{out} \times S}$.
