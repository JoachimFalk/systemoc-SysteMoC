...
As known from Kahn \cite{Kahn:1974} and Lee \cite{Lee98,LeeDenotialDF:1997} FIFOs are represented by the possible infinite sequence of tokens $\mathbf{v} \in V^{**}$ transmitted via them.
In order to simplify the mathematical description we will replace a bounded FIFO with two unbounded FIFOs.
...
a \emph{free space FIFO} $\mathbf{f} \in \{\circ\}^{**}$, where each \emph{free space token} $\circ$ represents an available slot in the value FIFO to store a token. % as known from PETRI-nets \cite{XXXX}.
Accordingly an actor not only consumes sequences of tokens $V^{**}$ on its input ports $I$ and produce them on its output ports $O$ but also produce sequences of \emph{free space tokens} $\{\circ\}^{**}$ on its input ports and consume them on its output ports, respectively.
...
Each firing step consumes a tuple of available input token sequences $\mathbf{s}_\mathrm{cons} \in \mathbf{S}_\mathrm{in} = (V^{**})^{|I|} \times (\{\circ\}^{**})^{|O|}$ and produces a tuple of sequences $\mathbf{s}_\mathrm{prod} \in \mathbf{S}_\mathrm{prod} = (\{\circ\}^{**})^{|I|} \times (V^{**})^{|O|}$.
Furthermore each firing step transforms the actor state $s \in S = \mathcal{F}.S_\mathrm{func} \times \mathcal{R}.S_\mathrm{firing}$, i.e., $\mathrm{Fire}: \mathbf{S}_\mathrm{in} \times a.S \to  \mathbf{S}_\mathrm{prod} \times a.S$.

\begin{definition}[Activation pattern]\label{def:activation-pattern}
  An \emph{activation pattern} $k$ of an actor $a \in A$ is a is a boolean function depending on the available input token sequences $\mathbf{S}_\mathrm{in}$ and the \emph{functionality state} $a.\mathcal{F}.S_\mathrm{func}$ of the actor, i.e., $k: \mathbf{S}_\mathrm{in} \times a.\mathcal{F}.S_\mathrm{func} \to \{\mathrm{true}, \mathrm{false}\}$.
...
\begin{eqnarray}
\mathrm{Fire}: Z^{|a.P|} \times a.\mathcal{F}.S_\mathrm{func} \times a.\mathcal{R}.S_\mathrm{firing} \to Z^{|a.P|} \times  a.\mathcal{F}.S_\mathrm{func} \times a.\mathcal{R}.S_\mathrm{firing}
\end{eqnarray}
