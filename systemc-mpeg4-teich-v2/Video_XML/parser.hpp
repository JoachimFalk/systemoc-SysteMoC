...
private: 
   ...
   return( (n == 1) ? thisb : (cal_bitor(lshift(thisb, (n - 1)), value(bits, (n - 1), (os + 1)))) );
   }
...
bool m_parser::guard_mb_dispatch_inter_no_ac(void)  const {
   ...
   return( (cal_bitand(cbp, lshift(1, (5 - comp))) == 0) );
}
...
// The action has 1 local variable declarations.
   ...
   cbpc = cal_bitand(rshift(mcbpc, 4), 3); 
   acpredflag = 0; 
}
...
// The action has 0 local variable declarations.
   ...
   mvcomp = 0; 
   cbpy = (15 - cbpy); 
   cbp = cal_bitor(lshift(cbpy, 2), cbpc); 
}
...
// The action has 0 local variable declarations.
   ac_coded = cal_bitand(cbp, lshift(1, (5 - comp))); 
   if (comp < 4) { 
      set_mvx(comp, 0);
      set_mvy(comp, 0);
   }
...
// The action has 3 local variable declarations.
   ...
   if (btype == INTRA) { 
      ...
      run = cal_bitand(rshift(val, 8), 255); 
      b_last = cal_bitand(rshift(val, 16), 1); 
      level = cal_bitand(val, 255); 
   }
   else { 
      val = inter_lookup[vld_index]; 
      run = cal_bitand(rshift(val, 4), 255); 
      b_last = cal_bitand(rshift(val, 12), 1); 
...
// The action has 3 local variable declarations.
   ...
   if (btype == INTRA) { 
      ...
      b_last = cal_bitand(rshift(val, 16), 1); 
      level = cal_bitand(val, 255); 
      run = (cal_bitand(rshift(val, 8), 255) + intra_max_run(b_last, level) + 1); 
   }
   else { 
      val = inter_lookup[vld_index]; 
      b_last = cal_bitand(rshift(val, 12), 1); 
      level = cal_bitand(val, 15); 
      run = (cal_bitand(rshift(val, 4), 255) + inter_max_run(b_last, level) + 1); 
   }
   b_index = (b_index + run); 
   block[b_index] = (sign == 1) ? (-level) : (level);
   b_index = (b_index + 1); 
