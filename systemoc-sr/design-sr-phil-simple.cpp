...
public:
     ...
     d11("d11")
  {
    ...
    TimerSignal().connect(timer5.timer)
      .connect(d6.in);
    TimerSignal().connect(d6.out)
      .connect(phil5.timer); 
    TimerSignal().connect(d6.history)
      .connect(phil5.timer_hist);
    
    TimerSignal().connect(timer4.timer)
      .connect(d7.in);
    TimerSignal().connect(d7.out)
      .connect(phil4.timer); 
    TimerSignal().connect(d7.history)
      .connect(phil4.timer_hist);
    
    TimerSignal().connect(timer3.timer)
      .connect(d8.in);
    TimerSignal().connect(d8.out)
      .connect(phil3.timer); 
    TimerSignal().connect(d8.history)
      .connect(phil3.timer_hist);

    TimerSignal().connect(timer2.timer)
      .connect(d9.in);
    TimerSignal().connect(d9.out)
      .connect(phil2.timer); 
    TimerSignal().connect(d9.history)
      .connect(phil2.timer_hist);

    TimerSignal().connect(timer1.timer)
      .connect(d10.in);
    TimerSignal().connect(d10.out)
      .connect(phil1.timer); 
    TimerSignal().connect(d10.history)
      .connect(phil1.timer_hist);

    BoolSignal().connect(clk.out)
      .connect(d11.in);
