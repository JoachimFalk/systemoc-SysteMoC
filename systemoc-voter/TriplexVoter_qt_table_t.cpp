...
#include "TriplexVoter_qt_table_t.hpp"
...
class m_source_qt_table_t: public smoc_actor {
  public:
  ...
  public:
    m_source_qt_table_t( sc_module_name name )
      :smoc_actor( name, start ) {
      start =  out(1) >> CALL(m_source_qt_table_t::process) >> start;
    }
};

class m_sink_qt_table_t: public smoc_actor {
  public:
  ...
  private:
    ...
    smoc_firing_state start;
  public:
    m_sink_qt_table_t( sc_module_name name )
      :smoc_actor( name, start ) {
      start = in(1) >> CALL(m_sink_qt_table_t::process) >> start;
    }
};
...
private:
  ...
  m_source_qt_table_t src_1;
  m_source_qt_table_t src_2;
  m_source_qt_table_t src_3;
  m_sink_bool sink_bool;
  m_sink_qt_table_t sink;
  TriplexVoter voter;
