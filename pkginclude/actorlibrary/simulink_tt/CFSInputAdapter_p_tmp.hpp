...
template<typename T, int PORTS=1>
 class CFSInputAdapter_p_tmp: public smoc_periodic_actor {
public:
  //smoc_port_in<T>  in;
  smoc_port_out<T>  out[PORTS];
struct sembuf operation[1];

  CFSInputAdapter_p_tmp( sc_module_name name, sc_time per, sc_time off, int _port )
    : smoc_periodic_actor(name, start, per, off),index(_port), sim_step() {
	

    Expr::Ex<bool >::type eIn(out[0](1) );
    ...
    start = //Expr::till( this->getEvent() )  >>
      //out(1)     >> //in (1)     >> 
      eIn >>
      CALL(CFSInputAdapter_p_tmp::process) >> start
      ;
...
protected:
  ...
  void process() {
	...
	//	sleep(0.0000000000001);
		//sleep(0.001);	
        shm_c_ptr[1] = sc_time_stamp().to_default_time_units();
        int i = 0;
        for(i=0;i<PORTS;i++)	
	   out[i][0] = shm_v_ptr[0];
	//cout << "lll";
