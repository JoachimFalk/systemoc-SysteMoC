...
#ifndef __INCLUDED__LOOKUP__HPP__
#define __INCLUDED__LOOKUP__HPP__



template<typename T>
 class Lookup: public smoc_actor {
public:
  smoc_port_in<T>  in;
  smoc_port_out<T>  out;

  Lookup( sc_module_name name, int* inputs, int* table, int length )
    : smoc_actor(name, start), inputs(inputs), table(table), length(length){


    start = in(1)              >>
      out(1)                   >>
      CALL(Lookup::process) >> start
      ;
  }
