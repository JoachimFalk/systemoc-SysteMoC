...
template<typename T>
 class CFSDelayTracker: public smoc_actor {
public:
  smoc_port_in<T>  in;
  //smoc_port_out<T>  out;
...
T Integration;
  ...
  CFSDelayTracker( sc_module_name name, int _port )
    : smoc_actor(name, start),index(_port), sim_step() {
	

    start = 
      in(1)     >> //in (1)     >>
      CALL(CFSDelayTracker::process) >> start
      ;
...
protected:
  ...
  void process() {
	...
	shm_c_ptr[1] = sc_time_stamp() - shm_c_ptr[1];  // Computing delay
	cout << "- " << shm_c_ptr[1] << endl;
	#endif
