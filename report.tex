...
discussions. This model consist of concurrent \emph{processes} which
can only communicate with each other via \emph{signals}. Before we can review
the tagged signal model some mathematic notations are needed.

Let $\{x_1,x_2\}$ denote a set without any order and
$[y_1,y_2,y_3,\ldots]$ denote a totally ordered set with total order
$y_1 \le y_2 \le y_3 \le \ldots$, e.g. $\{x_1,x_2\} = \{x_2,x_1\}$ but
$[y_1,y_2] \ne [y_2,y_1]$ and $\{x_1,x_1\} = \{x_1\}$ but $[ y_1, y_2, y_1, y_3]$
is a illformed totally ordered set because it does not define a total order,
e.g. $y_1 \le y_2 \wedge y_2 \le y_1$ but $y_1 \ne y_2$.

Let $\mathbb{Z}$ denote the set of integers,
$[1,2,3,\ldots] = \mathbb{N}$ the set of natural numbers,
$[1,2,3,\ldots\infty] = \mathbb{N}^\infty$ the set of natural numbers including infinity,
%$[\ldots,-3,-2,-1] = \mathbb{Z}^-$ the set of negative integers,
$[0,1,2,3,\ldots] = \mathbb{Z}^+_0$ the set of nonnegative integers,
$[0,1,2,3,\ldots\infty] = \mathbb{Z}^{+\infty}_0$ the set of nonnegative integers including infinity,
%$[\ldots,-3,-2,-1,0] = \mathbb{Z}^-_0$ the set of nonpositive integers,
and $[1,2,3,\ldots,n] = \mathbb{N}_n \subseteq \mathbb{N}^\infty$ the counting set from one to $n$
...
i.e., $\exists{\Sigma(X): Z_X \to X, \Sigma(X)\textrm{ is a \emph{bijection}}}:
 \forall{i,j \in Z_X}: i < j \iff \Sigma(X)(i) \le \Sigma(X)(j)$.
...
i.e., $\exists{\Sigma(X): \mathbb{N}_{|X|} \to X, \Sigma(X)\textrm{ is a \emph{bijection}}}:
 \forall{i,j \in \mathbb{N}_{|X|}}: i < j \iff \Sigma(X)(i) \le \Sigma(X)(j)$.
...
\caption{\label{fig:example-tsm-composition}
  Composition of two processes $\mathbf{P}_1$, $\mathbf{P}_2$,
...
\begin{equation}\label{equ:process-for-sequence-determinate-example}
\begin{array}{rll}
...
\begin{example}
The process $\mathbf{P}_1$  given in Equation~(\ref{equ:process-for-sequence-determinate-example})
is sequence determinate, whereas the processes $\mathbf{P}_2$ and
$\mathbf{P}_3$ given in Equation~(\ref{equ:process-for-sequence-determinate-example})
are not sequence determinate.
\end{example}
...
\begin{example}
The process given in Equation~(\ref{equ:process-for-input-example})
...
\caption{\label{fig:example-csp-tsm}
  ...
  communication between these two sequential processes.}
\end{figure}
...
$\mathbf{v} \in V^{**}$ where every sequence in $X$ is a prefix of $\mathbf{x}$,
i.e., $\forall{\mathbf{x} \in X}: \mathbf{v} \sqsubseteq \mathbf{x}$.
...
\emph{bottom element} of a poset $X$, if it exists, is element of the
...
\begin{figure}
\centering
%\includegraphics[scale=1]{fig:ng-petri.\graphicPostfix}\\
...
\begin{figure}
\centering
%\includegraphics[scale=1]{fig:ng-dataflow.\graphicPostfix}\\
...
\begin{definition}
  A \emph{node functionality} is a tupple $\mathcal{F} = (F,Q,q_1)$
  containing a set of functions $F$, a set of functionality states $Q$
  (possibly infinite), and an initial state $q_1$.
\end{definition}

The functions $f \in F$ of the node functionality map a
fixed number of input values $(v_{i1},v_{i2},\ldots,v_{iN}) = \mathbf{v}_i \in V^N$
...
programming language but the user must guarantee, that the functions
(i) terminate, and (ii) do not communicate with other actors.
...
which receive them from the channels connected to the actor input ports. The
firing rules store the input values in temporary buffers in these actor input ports
for later usage by the functions of the node functionality. The output values
$v_{o1}, v_{o2}, \ldots$ are stored by the functions of the node functionality
in temporary buffers in the actor output ports. These temporary buffers
are sent to the channels connected to these actor output ports by the
firing rules.
