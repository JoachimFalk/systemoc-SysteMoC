...
\end{verbatim}
\caption{Example of an node functionality
...
\begin{definition}\label{firing-transition}
  An \emph{firing transition} is a tuple $\mathbf{t} = (k,J)$ containing
...
via the actor input port $p_I$ and stores them into the
temporary buffers in these actor input ports. For actor output ports
$p_O$ the $k(p_O)$ tokens in the temporary buffers of the actor
output ports are send via these ports.
...
successor state, i.e., $|J.Q_\mathrm{succ}| \ge 1$.
...
a runtime decission of the function $J.f$.
