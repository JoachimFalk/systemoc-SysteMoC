...
\hypersetup{
  ...
  pdfauthor  = {\textcopyright\ Joachim Falk, Christian Haubelt, Jürgen Teich},
  colorlinks = true,
  linkcolor  = black,
  anchorcolor= black,
  citecolor  = black,
...
{Joachim Falk, Christian Haubelt, Jürgen Teich}{\today}
\clearpage
\tableofcontents
\clearpage
...
and the type of channel used for communication. Furthermore, a
concept is distinguished into \emph{concept types} by different
constraints placed on the concept. In the following, the concepts into
which the executable specification is separated and their further
differentiation into concept types is discussed in depth:
...
There is the part called \emph{node functionality} which is responsible
...
The node functionality is the only concept which is not further distinguished
into concept types.

\subsection{Node Interface Types}\label{node-interface-types}

The second part of an actor which determines the communication
behavior of the actor is called \emph{firing rules}. The firing rules
determine for each actor port the number of tokens to receive or send
until a node functionality can be invoked. These firing rules are
constructed by the use of communication operations which are
provided by the \emph{node interface}. Furthermore, we
distinguish three separate node interface types by the set
operations availlable to them:

\begin{concepttype}\label{choice-node-interface}
  Choice node interface

  \emph{Choice} is a operation defined on a set of actor ports $A_{P}$
  and a function $D: A_{P} \to \mathcal{F}$, which maps an actor port
  $a_{p} \in A_{P}$ to a function $F_{x} \in \mathcal{F}$.
  Let $A_{P_{rt}} \subseteq A_{P}$ be the subset of actor ports which
  are ready to communicate at time $t$. The choice operation
  blocks the process until time $t$ where at least one actor port
  is ready to communicate $|A_{P_{rt}}| \ge 1$. One actor port $a_{p} \in A_{P_{rt}}$
  will be chosen nondeterministically and the execution will contiune
  with function $F_{continue} = D(a_{p})$.
\end{concepttype}

\begin{concepttype}\label{transact-node-interface}
  Transact node interface
\end{concepttype}

\begin{concepttype}\label{fixed-transact-node-interface}
  Fixed transact node interface
\end{concepttype}

\subsection{Network Graph Types}\label{network-graph-types} 
...
\begin{concepttype}\label{petri-network-graph}
  Petri network graph is constrained such that exactly
...
\begin{concepttype}\label{dataflow-network-graph}
  Dataflow network graph is petri network graph with the additional constraint,
...
\subsection{Channel Kind Types}\label{channel-kind-types}
...
\begin{concepttype}\label{channel-kind-fifo} FIFO:
...
onto the concepts presented in Section~\ref{concept-overview}.
...
is provided as a operation in the choice node interface (See~\ref{choice-node-interface}).
...
(Subsection~\ref{node-functionality} and Subsection~\ref{node-interface-types}).
...
Subsection~\ref{network-graph-types}.
...
in Section~\ref{concept-overview}. Furthermore, the analysis
