...
\item A communication method for Hoare's so-called \emph{general choice}
  ...
  General choice written $a_{p1} \to F_1 \Box a_{p2} \to F_2$
  is a operation defined on a set of actor ports $A_{P}$
  ...
  are ready to communicate at time $t$. The general choice operation
...
\item Parallel executing processes
  ...
  The CSP parallel composition operation written $F_1 \| F_2$ executes the
...
\item The actors must only communicate over their ports with each other.
\end{enumerate}
...
\item The node interface must provide a communication operator which
      implements a 'general choice' communication.
...
\item The Parallel executing processes created by the init functions are mapped
      to the concurrently executing actors.
