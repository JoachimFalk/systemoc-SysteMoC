...
In the following, a example of a MAC unit for stereo signals is given
as a SDF system. The unit reads two tokens from input $in_1$,
multiplies this tokens both with a value read from $in_2$,
and accumulates these scaled values in the channels $c_2$ and
$c_3$. The accumulated values are written to $o_1$ for each
token pair received on $in_1$.
In Figure~\ref{fig:sdf-mac-unit} the annotation
at channel start and channel end points
of the constant consumption and production rates of the SDF
MoC can be seen.
