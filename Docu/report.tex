...
of MoCs. The values represent data which can be manipulated by computations.
...
\caption{\label{fig:example-tsm-composition}
  Display of the two processes $\mathbf{P}_1$ and $\mathbf{P}_2$
...
\begin{displaymath}
\begin{array}{rcl}
  \mathbf{P} & = & \{(\{(t_1,0),(t_2,0)\},\emptyset), (\{(t_1,1),(t_2,0)\},\{(t_1,0)\}),\\
             &   &   (\{(t_1,0),(t_2,1)\},\{(t_2,1)\}), (\{(t_1,1),(t_2,1)\},\{(t_1,0),(t_2,0)\})\} \subset S^2\\
  \mathbf{I} & = & \{\mathbf{s} \mid  \mathbf{s} \in S^2 \wedge \{(t_1,0),(t_2,1)\} = \pi{}_{(1)}(\mathbf{s})\}\\
  \mathbf{I} \cap \mathbf{P} & = & \{\mathbf{s} \mid  \mathbf{s} \in \mathbf{P} \wedge \{(t_1,0),(t_2,1)\} = \pi{}_{(1)}(\mathbf{s})\} =
                                   \{(\{(t_1,0),(t_2,1)\},\{(t_2,1)\})\}\\
\end{array}
\end{displaymath}
...
(iii) channels are connection processes.
