...
of MoCs. A totally ordered set of tags $T$ is called \emph{discrete} if it is
\emph{order isomorphic} to a subset of the integers, i.e.,
$\exists{f : T \to \{1,2,...|T|\}, f\textrm{ is a \emph{bijection}}}: \forall{t_1,t_2 \in T}: t_1 \le t_2 \iff f(t_1) \le f(t_2)$.
Intuitively this means, any two
...
%For a functional signal $s$ let $s(t)$ denote the one unique value $v$ if $(t,v) \in s$
%otherwise $s(t)$ is undefined.
% The signals mentioned in the following are all assumed to be functional.
...
of the i-th event in the signal $s$, i.e., $\Sigma(s)(i) = s(f^{-1}(i))$ where
$f^{-1}$ is the inverse function of the order isomorphism $f : T \to \{1,2,...|T|\}$.
...
% For example, consider a simple process that produces one output event
% for each input event. Suppose the input signal is , let the output be .
% Then the process imposes the ordering constraint that for all .
