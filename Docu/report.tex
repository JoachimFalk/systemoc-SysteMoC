...
transition is called \emph{conflict transition}. Which state $s \in J.S_{succ}$
...
(See Figure~\ref{example-adder-actor} and Figure~\ref{example-multiply-actor}
 method \code{transform} for an example of a node functionality).
...
by the Turing equivalent coding possibilities in the actor. To remedy these
problems we prohibit any method calls on \SysteMoC{} channels. Moreover,
communication over other media than \SysteMoC{} channels is forbidden but
this cannot be strictly enforced. So we require the \SysteMoC{} user
...
instead introduced firing rules which is a state machine which determines
the communication behaviour of the actor. However some MoCs, e.g. KPN, need
a turing equivalent control of their communication behaviour. To
...
constraint MoCs. Therefore, we have to distinguish firing rules into
...
have available for constucting them. These operations are provided by the
...
The choice node interface is the most capable in the hierarchy,
further down is the transact node interface, and at the bottom the
fixed transact node interface. This can also be clearly seen
by noticing that the operations available to node interface types
further down are a strict subset of the operation set available
to node interface types above them in the hierarchy. How these
execution and analysis capabilities map to some transformative
...
Node interface types further down in the hierarchy inherit
from ones above them. Operations which are no longer available for
node interface types further down are disabled by declaring them private.
...
\caption{\label{node-interface-hierarchy}UML inheritance hierarchy of the network graph types}
\end{figure}
