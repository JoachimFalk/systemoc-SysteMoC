...
public:
		...
		: smoc_graph(name)
	{
		...
		m_wsdf_2D_dup4<T> &min_image_dup = 
			registerNode(new m_wsdf_2D_dup4<T>("min_image_dup", size_x, size_y));
...
#ifdef LARGE_BUFFER_SIZE
		indConnectNodePorts(seed_dup.out1, dilatation.orig_in, smoc_wsdf_edge<T>(size_y));
		indConnectNodePorts(seed_dup.out2, diff_image.in1, smoc_wsdf_edge<T>(size_y));
#else
		indConnectNodePorts(seed_dup.out1, dilatation.orig_in, smoc_wsdf_edge<T>(3));
...
#endif
		indConnectNodePorts(min_image_dup.out4, output_switch.in, smoc_wsdf_edge<T>(size_y));
