...
//If the following define is set, then only multi-dimensional
...
#else
    m_wsdf_2D_dup2<T> &dup_1D_1 = 
      registerNode(new m_wsdf_2D_dup2<T>("1D_dup2_1",size_x,size_y));
    m_wsdf_2D_dup2<T> &dup_1D_2 = 
      registerNode(new m_wsdf_2D_dup2<T>("1D_dup2_2",size_x,size_y));
#endif
...
#else
    indConnectNodePorts(image_max.out, dup_1D_1.in,smoc_wsdf_edge<T>(size_y));
    indConnectNodePorts(dup_1D_1.out1, dup_1D_2.in,
                        smoc_wsdf_edge<T>(size_y,
                                          ns_smoc_vector_init::ul_vector_init[0][size_y]));

#ifdef LARGE_BUFFER_SIZE
    indConnectNodePorts(dup_1D_1.out2, output_switch.max_in, smoc_wsdf_edge<T>(size_y));

    indConnectNodePorts(dup_1D_2.out1, seed_switch.max_in, smoc_wsdf_edge<T>(size_y));
    indConnectNodePorts(dup_1D_2.out2, input_switch.max_in, smoc_wsdf_edge<T>(size_y));
#else
    indConnectNodePorts(dup_1D_1.out2, output_switch.max_in, smoc_wsdf_edge<T>(1));

    indConnectNodePorts(dup_1D_2.out1, seed_switch.max_in, smoc_wsdf_edge<T>(1));
    indConnectNodePorts(dup_1D_2.out2, input_switch.max_in, smoc_wsdf_edge<T>(1));
#endif
