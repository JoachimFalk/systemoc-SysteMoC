...
\begin{cexample} Declaration of communication behavior
\begin{verbatim}
class SqrLoop: public smoc_actor {
  ...
  // Declaration of states for the firing FSM
...
\begin{cdefinition}\label{def:firing-fsm}
  ...
  Where $k \in \mathcal{K}$ is called an \emph{activation pattern}, which is a function from
  ...
  The activation pattern decides if a transition $\mathbf{t} \in \mathcal{T}$ can be taken $k(x, q_\mathrm{func}) = \mathrm{true}$,
