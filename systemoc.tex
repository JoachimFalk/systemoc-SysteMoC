...
The approximative square root algorithm in Figure~\ref{fig:ng-sqrroot} is stimulated by an infinite sequence of input values generated by the \code{Src} actor $A_1$.
...
Each \emph{network graph} is represented as a C++ class derived from the base class \code{smoc\_graph}, e.g., as seen in the following example.
...
public:
  SqrRoot( sc_module_name name )
    ...
    // a3.o1 -> a4.i1 using FIFO size 1 and
    // an initial sequence of 2
    connectNodePorts(approx.o1,  dup.i1,
                     smoc_fifo<double>(1) << 2 );
    // a4.o1 -> a3.i2 using FIFO standard size
...
A \emph{general network graph} is a directed bipartite graph $G_N=(\mathcal{A},C,E)$ containing
