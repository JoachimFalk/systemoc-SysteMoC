...
Therefore the communication behavior of an actor is encoded in its \emph{firing FSM}, e.g., as seen in Figure~\ref{fig:firing-rules-sqrloop}.
Directed edges between these firing states are called \emph{firing transitions} or \emph{transitions} for short.
...
The states of a firing FSM are called \emph{firing states}, which are represented in SysteMoC as instances of the \code{smoc\_firing\_state} class.
The firing FSM of an actor is assembled in the constructor of the corresponding actor class by assigning each firing state its set of leaving transitions, e.g., as seen in the following example, which shows the SysteMoC representation of the firing FSM of the \code{SqrLoop} actor $A_2$.
A visual representation of this FSM can also be seen in Figure~\ref{fig:firing-rules-sqrloop}.
...
public:
  ...
  SqrLoop(sc_module_name name)
    ...
    start =
      // transition t1
        // with input pattern requiring at lest one token
        // in the FIFO connected to input port i1
        i1(1)                               >>
        // with output pattern requiring at lest one free space
