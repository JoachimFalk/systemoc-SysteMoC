...
private:
  ...
  smoc_firing_state vol, vol2, stuck, vol3, vol4, vol5, vol6, vol7, vol9, vol10, vol12, vol14, vop, vop2, vop3, vop4, vop5, vop6, mb, mb2, mb3, mb4, blk, mvstate, tex, texdc, texac, tex1, vld, vld3, vld5, vld4, vld6, mv1, mv2, mv3, mv4, mv5; 
...
public:
  m_parser(sc_module_name name, int VOL_START_CODE_LENGTH, int VOL_START_CODE, int VOP_START_CODE_LENGTH, int VOP_START_CODE, int B_VOP, int P_VOP, int I_VOP, int BITS_QUANT, int MCBPC_LENGTH, int INTER, int INTRA, int MAXW_IN_MB)
    ...
    vop.addTransition((bits.getAvailableTokens() >= (8-(var(bit_count)&7))) >> 
    ...
    mb4.addTransition((guard(&m_parser::guard_final_cbpy_inter)) >> 
		      call(&m_parser::final_cbpy_inter) >> mvstate); 
    ...
    mv5.addTransition((Expr::literal(true)) >> 
		      call(&m_parser::mvcompute) >> mvstate); 
