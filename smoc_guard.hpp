...
private:
  ...
  OpType     op;
public:
  DNodeNonTerminal(PDNodeBase l, PDNodeBase r, OpType op)
    : l(l), r(r), op(op) {}
...
std::ostream &operator << (std::ostream &o, const DNodeNonTerminal::OpType &op ) {
  switch (op) {
    ...
    case DNodeNonTerminal::DOpGt:       o << "DOpGe"; break;
...
//  bool      satisfied() const { return isAnd<filterSatisfied_ty>(); }
  ...
  bool      knownSatisfiable() const { return false; }
  bool      knownUnsatisfiable() const { return true; }
  ...
  template <class E>
  smoc_activation_pattern(const DExpr<E> &e) {}
